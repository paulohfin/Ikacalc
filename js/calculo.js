...
/*
	recebe o ID do select de pesquisa e alimenta com os números de 1 a 25
*/
function pesq(id){
	var el = document.getElementById(id);
	if(el.length > 1)
...
function salvar(){
	...
	txt += '[c],' + document.getElementById('pnm').options[document.getElementById('pnm').selectedIndex].value + ',' + document.getElementById('pnf').options[document.getElementById('pnf').selectedIndex].value +
		'\n[c],' + document.getElementById('pec').options[document.getElementById('pec').selectedIndex].value + ',' + document.getElementById('pef').options[document.getElementById('pef').selectedIndex].value + 
		'\n[c],' + document.getElementById('pci').options[document.getElementById('pci').selectedIndex].value  + ',' + document.getElementById('pcf').options[document.getElementById('pcf').selectedIndex].value +
		'\n[c],' + document.getElementById('pmi').options[document.getElementById('pmi').selectedIndex].value + ',' + document.getElementById('pmf').options[document.getElementById('pmf').selectedIndex].value;
	doc.text(35, 25, txt);
	doc.save('player.pdf');
}
/*
...
/*
	Função atualiza pesquisa
	recebe vetor, select e levelFutura
	atualiza html
*/
function atualizapesquisa(city, item, fut){
	var item = city.split(',');
	var j;
	item.shift();
	for(j = 0; j < city.length; j++)
		if(city[j].indexOf('\n') != -1){
			city[j] = city[j].replace('\n', '');
		}
	document.getElementById(item).value = item[0];
	document.getElementById(fut).value = item[1];
}
/*
...
function listar(texto){
	...
	atualizapesquisa(city[13], 'pnm', 'pnf');
	atualizapesquisa(city[14], 'pec', 'pef');
	atualizapesquisa(city[15], 'pci', 'pcf');
	atualizapesquisa(city[16], 'pmi', 'pmf');
}
function editar(id){
